<div class="side-nav dark bg-dark max-h-screen flex flex-col fixed top-0 left-0 z-40 pt-0.5 overflow-auto sidenav-scrollbar">
    <div class="flex flex-col h-full">
        <div class="flex flex-col h-full ms-1 mb-2 me-0">
            <x-examples.navigation.company-dropdown/>
            <div class="flex flex-col justify-between h-full px-1">
                <ul>
                    <li id="home" class="menu-item menu-item-hoverable h-6 px-0 mb-2">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Hjem">
                                <x-examples.icons.home/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Hjem</p>
                        </a>
                    </li>
                    <li id="status-dash" class="menu-item menu-item-hoverable h-6 px-0 mb-2">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--{{ route('status-dashboard.index') }}--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Status dashboard">
                                <x-examples.icons.dashboard/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Status dashboard</p>
                        </a>
                    </li>
                    <li id="employee-board" class="menu-item menu-item-hoverable h-6 px-0 mb-2">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--{{ route('employee-board.index') }}--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Medarbejder board">
                                <x-examples.icons.dashboard/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Medarbejder
                                board</p>
                        </a>
                    </li>
                    <li class="side-nav-divider mt-2 border-gray-500"></li>
                    <li id="customer" class="menu-item menu-item-hoverable px-0 h-6 mb-2 side-nav-item">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/crm/1/customers--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Kunder">
                                <x-examples.icons.customer/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Kunder</p>
                        </a>
                    </li>
                    <li id="quotes" class="menu-item menu-item-hoverable px-0 h-6 mb-2 side-nav-item">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/quotes--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Tilbud">
                                <x-examples.icons.quote/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Tilbud</p>
                        </a>
                    </li>
                    <li id="orders" class="menu-item menu-item-hoverable px-0 h-6 mb-2 side-nav-item">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/orders--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Sager">
                                <x-examples.icons.case/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Sager</p>
                        </a>
                    </li>
                    <li id="planning" class="menu-item menu-item-hoverable px-0 h-6 mb-2 side-nav-item">
                        <a
                            class="flex h-full w-full px-2 gap-2 items-center side-nav-list font-normal"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/planner--}}#"
                        >
                            <div class="icon-sm stroke-white fill-white" title="Planlægning">
                                <x-examples.icons.planning/>
                            </div>
                            <p class="text-gray-100" x-show="sideNavOpen" x-transition x-cloak>Planlægning</p>
                        </a>
                    </li>
                </ul>
            </div>
            <div class="mt-16 px-1">
                <ul>
                    <li id="service-templates" class="menu-item px-0 h-6 side-nav-item"
                        x-bind:class="sideNavOpen ? 'hover:underline' : 'menu-item-hoverable'">
                        <a
                            class="flex w-full px-2 gap-2 items-center side-nav-list"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/service-templates--}}#"
                        >
                            <div class="icon-xs stroke-white fill-white" title="Serviceskabeloner">
                                <x-examples.icons.service/>
                            </div>
                            <p class="text-xs text-gray-100 font-light" x-show="sideNavOpen" x-transition x-cloak>
                                Serviceskabeloner</p>
                        </a>
                    </li>
                    <li id="employees" class="menu-item px-0 h-6 side-nav-item"
                        x-bind:class="sideNavOpen ? 'hover:underline' : 'menu-item-hoverable'">
                        <a
                            class="flex w-full px-2 gap-2 items-center side-nav-list"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/employees--}}#"
                        >
                            <div class="icon-xs stroke-white fill-white" title="Medarbejdere">
                                <x-examples.icons.employees/>
                            </div>
                            <p class="text-xs text-gray-100 font-light" x-show="sideNavOpen" x-transition x-cloak>
                                Medarbejdere</p>
                        </a>
                    </li>
                    <li id="equipment" class="menu-item px-0 h-6 side-nav-item"
                        x-bind:class="sideNavOpen ? 'hover:underline' : 'menu-item-hoverable'">
                        <a
                            class="flex w-full px-2 gap-2 items-center side-nav-list"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/equipment/1/types--}}#"
                        >
                            <div class="icon-xs stroke-white fill-white" title="Materiel">
                                <x-examples.icons.equipment/>
                            </div>
                            <p class="text-xs text-gray-100 font-light" x-show="sideNavOpen" x-transition x-cloak>
                                Materiel</p>
                        </a>
                    </li>
                    <li id="certificates" class="menu-item px-0 h-6 side-nav-item"
                        x-bind:class="sideNavOpen ? 'hover:underline' : 'menu-item-hoverable'">
                        <a
                            class="flex w-full px-2 gap-2 items-center side-nav-list"
                            x-bind:class="(sideNavOpen || centeringDelay) ? 'justify-start' : 'justify-center'"
                            href="{{--/certificates/1/types--}}#"
                        >
                            <div class="icon-xs stroke-white fill-white" title="Certifikater">
                                <x-examples.icons.certificate/>
                            </div>
                            <p class="text-xs text-gray-100 font-light" x-show="sideNavOpen" x-transition x-cloak>
                                Certifikater</p>
                        </a>
                    </li>
                    <li class="side-nav-divider mt-2 border-gray-500"></li>
                </ul>
                <div class="flex justify-center">
                    <div x-bind:class="(sideNavOpen) ? 'justify-start' : 'justify-center'">
                        <img class="h-6" src="{{ asset('images/logo-icons/white.svg') }}"
                             alt="logo-icons/white.svg" x-show="!sideNavOpen" x-cloak>
                        <img class="h-6" src="{{ asset('images/logos/white.svg') }}" alt="logos/white.svg"
                             x-show="sideNavOpen" x-cloak>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>


