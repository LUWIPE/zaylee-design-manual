<div class="relative" x-data="{ open: false }" x-on:click.away="open = false">
    ...
    <div class="w-full rounded-md rounded-t-none border border-t-0 absolute z-10 bg-gray-100" x-show="open">
        <select class="w-full p-1 bg-gray-100 custom-scrollbar-dark focus:outline-none" multiple>
            {{--@php $quoteStatuses = QuoteStatus::from($quote->status);
            @endphp
            @foreach($quoteStatuses as $quoteStatus)
                <option value="{{ $quoteStatus }}">{{ $quoteStatus }}</option>
            @endforeach--}}
            <option class="select-option text-sm h-4 my-0.5" value="1">Status 1</option>
            <option class="select-option text-sm h-4 my-0.5" value="2">Status 2</option>
        </select>
    </div>
</div>
